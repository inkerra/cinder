...
class API(base.Base):
    ...
    def _aggregate_permission(self, perms):
        res = access_levels.NoPermission
        for perm in perms:
            addendum = perm.access_permission
            if addendum == access_levels.FullAccess:
                return addendum
            if res == addendum:
                continue
            res = access_levels.aggregate(res, addendum)
            if res == access_levels.FullAccess:
                return res
    ...
    def get_access(self, cxt, volume_id):
        ...
        return access_levels.NoPermission
    ...
    def _volume_permission_has_read_perm_access(self, cxt, vol_id):
        return self._volume_permission_has_perm_access(
            cxt, vol_id,
            lambda p: p.access_permission >= access_levels.PermissionRead)

    def _volume_permission_has_write_perm_access(self, cxt, vol_id):
        return self._volume_permission_has_perm_access(
            cxt, vol_id,
            lambda p: p.access_permission in (access_levels.PermissionWrite,
